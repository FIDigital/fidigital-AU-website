export const metadata = {
  title: "SEO Melbourne | Custom Search Engine Optimisation Strategies | FI Digital",
  description: "Boost your online visibility, improve organic search rankings, and attract targeted web traffic with FI Digital's expert SEO campaigns in Melbourne.",
  keywords: [
    "SEO Melbourne",
    "Search Engine Optimisation Australia",
    "Organic Search Rankings",
    "Custom SEO Strategy",
    "Local SEO Marketing"
  ],
  metadataBase: new URL('https://fidigital.com.au'),
  alternates: {
      canonical: 'https://fidigital.com.au/search-engine-optimisation',
  },
  openGraph: {
    title: 'Melbourne Search Engine Optimisation Services',
    description: 'Outrank competitors natively with FI Digital. Develop robust custom SEO strategies, climb Google rankings reliably, and grow your local business profile securely.',
    url: 'https://fidigital.com.au/search-engine-optimisation',
    siteName: "FI Digital",
    images: [
      {
        url: "/og-seo.jpg",
        width: 1200,
        height: 630,
        alt: "FI Digital - Melbourne SEO Services",
      },
    ],
    locale: "en_AU",
    type: "website",
  },
};
